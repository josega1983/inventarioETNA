...
public class ParametroFuncionalAction extends BaseAction<ParametroFuncionalDTO> implements SessionAware {
	/**
	 * Identificador de serializacion.
	 */
	private static final long serialVersionUID = -1118206119646826262L;
	/**
	 * Constante string que define el elemento sobre el trabaja este action.
	 */
	private static final String ELEMENTO = "Parámetro Funcional";
	/**
	 * Clave para almacenar el filtro de busqueda en la sesion.
	 */
	private final String FILTRO_BUSQUEDA_PARAMETRO_FUNCIONAL = "Filtro_busqueda_parametro_funcional";

	/**
	 * Clave para almacenar el filtro de busqueda del popup en la sesion.
	 */
	private final String FILTRO_BUSQUEDA_POPUP_PARAMETRO_FUNCIONAL = "Filtro_busqueda_popup_parametro_funcional";
	/**
	 * Lista de tipos de campos
	 */
	private List<TipoCampoParametroDTO> tipoCampoParametroList;
	/**
	 * Constructor
	 */
	public ParametroFuncionalAction() {
		elemento = new ParametroFuncionalDTO();
		nombreElemento = ELEMENTO;
		tipoEntrada = TipoEntradaConstants.PARAMETRO_FUNCIONAL;
		nombreFiltroSesion = FILTRO_BUSQUEDA_PARAMETRO_FUNCIONAL;
		nombreFiltroPopup = FILTRO_BUSQUEDA_POPUP_PARAMETRO_FUNCIONAL;
		setNamespace(ConstantesMigasPan.ESPACIO_PARAMETRO_FUNCIONAL);
		setFiltro(new FiltroParametroFuncional());
		tipoCampoParametroList = new ArrayList<TipoCampoParametroDTO>();
	 ...
	 */
	@Override
	public ParametroFuncionalDTO getModel() {
		if (!isInicializado()) {
			RespuestaDTO respuestaTipoCampoParametro = getFacadeBackend().list(new EntradaDTO(null,0L, 10L, TipoEntradaConstants.TIPO_CAMPO_PARAMETRO, null));
			if(respuestaTipoCampoParametro.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)){
				Object[] objetoRespuestaTipoCampoParametro = respuestaTipoCampoParametro.getObjetoRespuesta();
				if(objetoRespuestaTipoCampoParametro != null){
					for (Object object : objetoRespuestaTipoCampoParametro) {
						TipoCampoParametroDTO tipoCampoParametro = (TipoCampoParametroDTO) object;
						tipoCampoParametroList.add(tipoCampoParametro);
					}
				}
			}
		}
	...
	public String list(){
		...
		nombreMiga = ConstantesMigasPan.LISTADO_PARAMETRO_FUNCIONAL;
		return super.list();
	}
	...
	public void aplicarEstadoMenu(ParametroFuncionalDTO resultado) {
	...
	public String exportarList_PDF() {
		filtro = (FiltroParametroFuncional) getSession().get(FILTRO_BUSQUEDA_PARAMETRO_FUNCIONAL);

		try {
			EntradaDTO entrada = new EntradaDTO();
			entrada.setTipoEntrada(TipoEntradaConstants.PARAMETRO_FUNCIONAL);
			...
			setNombreInforme("ListadoParametrosFuncionales");
		} catch (Exception e) {
	...
	public String exportarList_XLS() {
		...
		try {
			...
			list = Utilidades.convertArrayToList(respuesta.getObjetoRespuesta());
				...
				InformeExcelDTO informeExcelDTO = ConverterInformeExcel.convertFromParametroFuncionals(list);
				...
				setContentDisposition("attachment; filename=ListadoParametrosFuncionales."+getDocumentFormat());
	...
	public String exportarShowMore() {
		try {
			...
			elemento =(ParametroFuncionalDTO) respuesta.getObjetoRespuesta()[0];
	...
	public String preExportarList() {
		filtro = (FiltroParametroFuncional) getSession().get(nombreFiltroSesion);
		if(filtro == null){
			filtro = new FiltroParametroFuncional();
		}
	...
	/**
	 * Obtiene la Lista de tipos de campos
	 * @return la Lista de tipos de campos
	 */
	public List<TipoCampoParametroDTO> getTipoCampoParametroList() {
		return tipoCampoParametroList;
	}
	/**
	 * Establece la Lista de tipos de campos
	 * @param tipoCampoParametroList la Lista de tipos de campos
	 */
	public void setTipoCampoParametroList(List<TipoCampoParametroDTO> tipoCampoParametroList) {
		this.tipoCampoParametroList = tipoCampoParametroList;
	}
