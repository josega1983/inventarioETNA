...
public class PropietarioActivoAction extends BaseAction<PropietarioActivoDTO> implements SessionAware, IInformes {
	/**
	 * Identificador de serializacion.
	 */
	private static final long serialVersionUID = 4082727107538265224L;
	/**
	 * Constante string que define el elemento sobre el trabaja este action.
	 */
	private static final String ELEMENTO = "Propietario Activo";
	/**
	 * Clave para almacenar el filtro de busqueda en la sesion.
	 */
	private final String FILTRO_BUSQUEDA_PROPIETARIO_ACTIVO = "Filtro_busqueda_propietario_activo";

	/**
	 * Clave para almacenar el filtro de busqueda del popup en la sesion.
	 */
	private final String FILTRO_BUSQUEDA_POPUP_PROPIETARIO_ACTIVO = "Filtro_busqueda_popup_propietario_activo";
	/**
	 * Constructor
	 */
	public PropietarioActivoAction() {
		elemento = new PropietarioActivoDTO();
		nombreElemento = ELEMENTO;
		tipoEntrada = TipoEntradaConstants.PROPIETARIO_ACTIVO;
		nombreFiltroSesion = FILTRO_BUSQUEDA_PROPIETARIO_ACTIVO;
		nombreFiltroPopup = FILTRO_BUSQUEDA_POPUP_PROPIETARIO_ACTIVO;
		setNamespace(ConstantesMigasPan.ESPACIO_PROPIETARIO_ACTIVO);
		setFiltro(new FiltroPropietarioActivo());
	 ...
	 */
	@Override
	public PropietarioActivoDTO getModel() {
		return elemento;
	}
	/**
	...
	public String list(){
		...
		nombreMiga = ConstantesMigasPan.LISTADO_PROPIETARIO_ACTIVO;
		return super.list();
	}
	...
	public void aplicarEstadoMenu(PropietarioActivoDTO resultado) {
	...
	public String exportarList_PDF() {
		filtro = (FiltroPropietarioActivo) getSession().get(FILTRO_BUSQUEDA_PROPIETARIO_ACTIVO);

		try {
			EntradaDTO entrada = new EntradaDTO();
			entrada.setTipoEntrada(TipoEntradaConstants.PROPIETARIO_ACTIVO);
			...
			setNombreInforme("ListadoPropietariosActivos");
		} catch (Exception e) {
	...
	public String exportarList_XLS() {
		...
		try {
			...
			list = Utilidades.convertArrayToList(respuesta.getObjetoRespuesta());
				...
				InformeExcelDTO informeExcelDTO = ConverterInformeExcel.convertFromPropietarioActivos(list);
				...
				setContentDisposition("attachment; filename=ListadoPropietarioActivos."+getDocumentFormat());
	...
	public String preExportarList() {
		filtro = (FiltroPropietarioActivo) getSession().get(nombreFiltroSesion);
		if(filtro == null){
			filtro = new FiltroPropietarioActivo();
		}
	...
	public String exportarShowMore() {
		try {
			...
			elemento =(PropietarioActivoDTO) respuesta.getObjetoRespuesta()[0];
			setNombreInforme("DetallePropietarioActivo"+ elemento.getId());
		} catch (Exception e) {
			loggerINVENTARIO.error(this.getClass().toString() + ":" + Thread.currentThread().getStackTrace()[1].getMethodName(),e);
			return ERROR;
		}
