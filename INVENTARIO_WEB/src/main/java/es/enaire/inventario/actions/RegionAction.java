...
public class RegionAction extends BaseAction<RegionDTO> implements SessionAware, IInformes {
	 ...
	 */
	private static final String ELEMENTO = "Región";
	/**
	 * Clave para almacenar el filtro de busqueda en la sesion.
	 */
	private final String FILTRO_BUSQUEDA_REGION = "Filtro_region_region";

	/**
	 * Clave para almacenar el filtro de busqueda del popup en la sesion.
	 */
	private final String FILTRO_BUSQUEDA_POPUP_REGION = "Filtro_busqueda_popup_region";
	/**
	 * Constructor
	 */
	public RegionAction() {
		elemento = new RegionDTO();
		nombreElemento = ELEMENTO;
		tipoEntrada = TipoEntradaConstants.REGION;
		nombreFiltroSesion = FILTRO_BUSQUEDA_REGION;
		nombreFiltroPopup = FILTRO_BUSQUEDA_POPUP_REGION;
		setNamespace(ConstantesMigasPan.ESPACIO_REGION);
		setFiltro(new FiltroRegion());
	 ...
	 */
	public String list(){
		nombreMiga = ConstantesMigasPan.LISTADO_REGIONES;
		return super.list();
	}
	...
	public void aplicarEstadoMenu(RegionDTO resultado) {
	...
	public String exportarList_PDF() {
		filtro = (FiltroRegion) getSession().get(FILTRO_BUSQUEDA_REGION);

		try {
			EntradaDTO entrada = new EntradaDTO();
			entrada.setTipoEntrada(TipoEntradaConstants.REGION);
			...
			setNombreInforme("ListadoRegiones");
		} catch (Exception e) {
	...
	public String exportarList_XLS() {
		...
		try {
			...
			list = Utilidades.convertArrayToList(respuesta.getObjetoRespuesta());
				...
				InformeExcelDTO informeExcelDTO = ConverterInformeExcel.convertFromRegiones(list);
				...
				setContentDisposition("attachment; filename=ListadoRegiones."+getDocumentFormat());
	...
	public String preExportarList() {
		filtro = (FiltroRegion) getSession().get(nombreFiltroSesion);
		if(filtro == null){
			filtro = new FiltroRegion();
		}
	...
	public String exportarShowMore() {
		try {
			...
			elemento =(RegionDTO) respuesta.getObjetoRespuesta()[0];
			setNombreInforme("DetalleRegion"+ elemento.getId());
		} catch (Exception e) {
			loggerINVENTARIO.error(this.getClass().toString() + ":" + Thread.currentThread().getStackTrace()[1].getMethodName(),e);
			return ERROR;
		}
