...
public class SectorAction extends BaseAction<SectorDTO> implements SessionAware, IInformes {

	/**
	 * Identificador de serializacion.
	 */
	private static final long serialVersionUID = 537384543901376981L;
	/**
	 * Constante string que define el elemento sobre el trabaja este action.
	 */
	private static final String ELEMENTO = "Sector";
	/**
	 * Clave para almacenar el filtro de busqueda en la sesion.
	 */
	private final String FILTRO_BUSQUEDA_SECTOR = "Filtro_sector_empresa";

	/**
	 * Clave para almacenar el filtro de busqueda del popup en la sesion.
	 */
	private final String FILTRO_BUSQUEDA_POPUP_SECTOR = "Filtro_busqueda_popup_sector";
	/**
	 * Constructor
	 */
	public SectorAction() {
		elemento = new SectorDTO();
		nombreElemento = ELEMENTO;
		tipoEntrada = TipoEntradaConstants.SECTOR;
		nombreFiltroSesion = FILTRO_BUSQUEDA_SECTOR;
		nombreFiltroPopup = FILTRO_BUSQUEDA_POPUP_SECTOR;
		setNamespace(ConstantesMigasPan.ESPACIO_SECTOR);
		setFiltro(new FiltroSector());
	...
	public String list(){
		...
		nombreMiga = ConstantesMigasPan.LISTADO_SECTORES;
		return super.list();
	}
	...
	public void aplicarEstadoMenu(SectorDTO resultado) {
	...
	public String exportarList_PDF() {
		filtro = (FiltroSector) getSession().get(FILTRO_BUSQUEDA_SECTOR);

		try {
			EntradaDTO entrada = new EntradaDTO();
			entrada.setTipoEntrada(TipoEntradaConstants.SECTOR);
			...
			setNombreInforme("ListadoSectores");
		} catch (Exception e) {
	...
	public String exportarList_XLS() {
		...
		try {
			...
			list = Utilidades.convertArrayToList(respuesta.getObjetoRespuesta());
				...
				InformeExcelDTO informeExcelDTO = ConverterInformeExcel.convertFromSectores(list);
				...
				setContentDisposition("attachment; filename=ListadoSectores."+getDocumentFormat());
	...
	public String preExportarList() {
		filtro = (FiltroSector) getSession().get(nombreFiltroSesion);
		if(filtro == null){
			filtro = new FiltroSector();
		}
	...
	public String exportarShowMore() {
		try {
			...
			elemento =(SectorDTO) respuesta.getObjetoRespuesta()[0];
			setNombreInforme("DetalleSector"+ elemento.getId());
		} catch (Exception e) {
			loggerINVENTARIO.error(this.getClass().toString() + ":" + Thread.currentThread().getStackTrace()[1].getMethodName(),e);
			return ERROR;
		}
