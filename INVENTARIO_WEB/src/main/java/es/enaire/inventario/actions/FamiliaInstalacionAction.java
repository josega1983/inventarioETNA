...
public class FamiliaInstalacionAction extends BaseAction<FamiliaInstalacionDTO> implements SessionAware, IInformes   {
	/**
	 * Identificador de serializacion.
	 */
	private static final long serialVersionUID = 8411266701847560859L;
	/**
	 * Constante string que define el elemento sobre el trabaja este action.
	 */
	private static final String ELEMENTO = "Familia Instalación";
	/**
	 * Clave para almacenar el filtro de busqueda en la sesion.
	 */
	private final String FILTRO_BUSQUEDA_FAMILIA_INSTALACION = "Filtro_busqueda_familia_instalacion";

	/**
	 * Clave para almacenar el filtro de busqueda del popup en la sesion.
	 */
	private final String FILTRO_BUSQUEDA_POPUP_FAMILIA_INSTALACION = "Filtro_busqueda_popup_familia_instalacion";
	/**
	 * Constructor
	 */
	public FamiliaInstalacionAction() {
		elemento = new FamiliaInstalacionDTO();
		nombreElemento = ELEMENTO;
		tipoEntrada = TipoEntradaConstants.FAMILIA_INSTALACION;
		nombreFiltroSesion = FILTRO_BUSQUEDA_FAMILIA_INSTALACION;
		nombreFiltroPopup = FILTRO_BUSQUEDA_POPUP_FAMILIA_INSTALACION;
		setNamespace(ConstantesMigasPan.ESPACIO_FAMILIA_INSTALACION);
		setFiltro(new FiltroFamiliaInstalacion());
	 ...
	 */
	@Override
	public FamiliaInstalacionDTO getModel() {
		return elemento;
	}
	/**
	 ...
	 */
	public String list(){
		nombreMiga = ConstantesMigasPan.LISTADO_FAMILIA_INSTALACION;
		return super.list();
	}
	...
	public void aplicarEstadoMenu(FamiliaInstalacionDTO resultado) {
	...
	public String exportarList_PDF() {
		filtro = (FiltroFamiliaInstalacion) getSession().get(FILTRO_BUSQUEDA_FAMILIA_INSTALACION);

		try {
			EntradaDTO entrada = new EntradaDTO();
			entrada.setTipoEntrada(TipoEntradaConstants.FAMILIA_INSTALACION);
			...
			setNombreInforme("ListadoFamiliaInstalacion");
		} catch (Exception e) {
	...
	public String exportarList_XLS() {
		...
		try {
			...
			list = Utilidades.convertArrayToList(respuesta.getObjetoRespuesta());
				...
				InformeExcelDTO informeExcelDTO = ConverterInformeExcel.convertFromFamiliaInstalacions(list);
				...
				setContentDisposition("attachment; filename=ListadoFamiliaInstalacions."+getDocumentFormat());
	...
	public String preExportarList() {
		filtro = (FiltroFamiliaInstalacion) getSession().get(nombreFiltroSesion);
		if(filtro == null){
			filtro = new FiltroFamiliaInstalacion();
		}
	...
	public String exportarShowMore() {
		try {
			...
			elemento =(FamiliaInstalacionDTO) respuesta.getObjetoRespuesta()[0];
			setNombreInforme("DetalleFamiliaInstalacion"+ elemento.getId());
		} catch (Exception e) {
			loggerINVENTARIO.error(this.getClass().toString() + ":" + Thread.currentThread().getStackTrace()[1].getMethodName(),e);
			return ERROR;
		}
