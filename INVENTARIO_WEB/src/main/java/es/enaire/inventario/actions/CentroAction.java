...
public class CentroAction extends BaseAction<CentroDTO> implements SessionAware, IInformes {

	/**
	 * Identificador de serializacion.
	 */
	private static final long serialVersionUID = -7858116502959285354L;
	/**
	 * Constante string que define el elemento sobre el trabaja este action.
	 */
	private static final String ELEMENTO = "Centro";
	/**
	 * Clave para almacenar el filtro de busqueda en la sesion.
	 */
	private final String FILTRO_BUSQUEDA_CENTRO = "Filtro_centro_empresa";

	/**
	 * Clave para almacenar el filtro de busqueda del popup en la sesion.
	 */
	private final String FILTRO_BUSQUEDA_POPUP_CENTRO = "Filtro_busqueda_popup_centro";
	/**
	 * Constructor
	 */
	public CentroAction() {
		elemento = new CentroDTO();
		nombreElemento = ELEMENTO;
		tipoEntrada = TipoEntradaConstants.CENTRO;
		nombreFiltroSesion = FILTRO_BUSQUEDA_CENTRO;
		nombreFiltroPopup = FILTRO_BUSQUEDA_POPUP_CENTRO;
		setNamespace(ConstantesMigasPan.ESPACIO_CENTRO);
		setFiltro(new FiltroCentro());
	...
	public String list(){
		...
		nombreMiga = ConstantesMigasPan.LISTADO_CENTROS;
	...
	public String preExportarList() {
		filtro = (FiltroCentro) getSession().get(nombreFiltroSesion);
		if(filtro == null){
			filtro = new FiltroCentro();
		}
		return super.preExportarList();
	}
	public void aplicarEstadoMenu(CentroDTO resultado) {
	...
	public String exportarList_PDF() {
		filtro = (FiltroCentro) getSession().get(FILTRO_BUSQUEDA_CENTRO);

		try {
			EntradaDTO entrada = new EntradaDTO();
			entrada.setTipoEntrada(TipoEntradaConstants.CENTRO);
			...
			setNombreInforme("ListadoCentros");
		} catch (Exception e) {
	...
	public String exportarList_XLS() {
		...
		try {
			...
			list = Utilidades.convertArrayToList(respuesta.getObjetoRespuesta());
				...
				InformeExcelDTO informeExcelDTO = ConverterInformeExcel.convertFromCentros(list);
				...
				setContentDisposition("attachment; filename=ListadoCentros."+getDocumentFormat());
	...
	public String exportarShowMore() {
		try {
			...
			elemento =(CentroDTO) respuesta.getObjetoRespuesta()[0];
			setNombreInforme("DetalleCentro"+ elemento.getId());
		} catch (Exception e) {
			loggerINVENTARIO.error(this.getClass().toString() + ":" + Thread.currentThread().getStackTrace()[1].getMethodName(),e);
			return ERROR;
		}
