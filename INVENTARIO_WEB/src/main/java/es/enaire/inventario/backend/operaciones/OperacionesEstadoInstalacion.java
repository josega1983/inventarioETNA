...
public class OperacionesEstadoInstalacion {

	/**
	 * DAO de estado Instalaciones
	 */
	@Autowired
	private EstadoInstalacionDAO<EstadoInstalacionDTO> estadoInstalacionDAO;
	/**
	...
	private RespuestaDTO validateInfo(EntradaDTO entrada) {
		...
		if(!(entrada.getObjetoEntrada() instanceof EstadoInstalacionDTO)){
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_TIPO_ENTRADA_NO_ESPERADA);
			return respuesta;
		}

		if(entrada.getObjetoEntrada() instanceof EstadoInstalacionDTO) {
			EstadoInstalacionDTO estadoInstalacion =(EstadoInstalacionDTO) entrada.getObjetoEntrada();
			if (estadoInstalacion.getId() == null) {
	...
	public RespuestaDTO info(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			...
			EstadoInstalacionDTO estadoInstalacionRespuesta = null;
			if(entrada.getObjetoEntrada() instanceof EstadoInstalacionDTO) {
				estadoInstalacionRespuesta = estadoInstalacionDAO.listById(((EstadoInstalacionDTO)entrada.getObjetoEntrada()).getId());
			}

			if(estadoInstalacionRespuesta != null) {
				Object[] objetoRespuesta={estadoInstalacionRespuesta};
	...
	public RespuestaDTO list(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			List<EstadoInstalacionDTO> estadoInstalacionRespuesta = estadoInstalacionDAO.list(entrada.getPaginaActual(), entrada.getTamanioPagina());
			Object[] objetoRespuesta = estadoInstalacionRespuesta.toArray();
			respuesta.setObjetoRespuesta(objetoRespuesta);
		}
		return respuesta;
	}
	...
	private RespuestaDTO validateEdit(EntradaDTO entrada) throws Exception {
		...
		EstadoInstalacionDTO estadoInstalacion = (EstadoInstalacionDTO) entrada.getObjetoEntrada();
		if(estadoInstalacion.getNombre() == null ||estadoInstalacion.getNombre().isEmpty() ){
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_NOMBRE_NO_INFORMADO);
			return respuesta;
		}
		FiltroEstadoInstalacion filtro = new FiltroEstadoInstalacion();
		filtro.setNombre(estadoInstalacion.getNombre());
		if(estadoInstalacion.getId() != null){//es una actualizacion y hay que excluirlo
			filtro.setIdExcluir(estadoInstalacion.getId());
		}
		List<EstadoInstalacionDTO> estadoInstalacionList = estadoInstalacionDAO.listByName(filtro.getNombre());
	...
	public RespuestaDTO edit(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			EstadoInstalacionDTO estadoInstalacionEntrada = (EstadoInstalacionDTO) entrada.getObjetoEntrada();
			EstadoInstalacionDTO estadoInstalacionRespuesta = estadoInstalacionDAO.saveOrUpdate(estadoInstalacionEntrada);
			Object[] objetoRespuesta={estadoInstalacionRespuesta};
			respuesta.setObjetoRespuesta(objetoRespuesta);
		}
		return respuesta;
	...
	public RespuestaDTO reactivar(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			EstadoInstalacionDTO estadoInstalacionEntrada = (EstadoInstalacionDTO) entrada.getObjetoEntrada();
			EstadoInstalacionDTO estadoInstalacionRespuesta = estadoInstalacionDAO.reactivar(estadoInstalacionEntrada.getId());
			Object[] objetoRespuesta={estadoInstalacionRespuesta};
			respuesta.setObjetoRespuesta(objetoRespuesta);
		}
		return respuesta;
	...
	public RespuestaDTO delete(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			EstadoInstalacionDTO estadoInstalacionEntrada = (EstadoInstalacionDTO) entrada.getObjetoEntrada();
			estadoInstalacionDAO.delete(estadoInstalacionEntrada.getId());
		}
		return respuesta;
	}
	/**
	...
	private RespuestaDTO validateSearch(EntradaDTO entrada) {
		...
		if(!(entrada.getObjetoEntrada() instanceof FiltroEstadoInstalacion)) {
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_TIPO_ENTRADA_NO_ESPERADA);
			return respuesta;
		}
	...
	public RespuestaDTO search(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			List<EstadoInstalacionDTO> regionListRespuesta = estadoInstalacionDAO.search( (FiltroEstadoInstalacion) entrada.getObjetoEntrada(), entrada.getTamanioPagina(), entrada.getPaginaActual());
	...
	public RespuestaDTO getCantidad(EntradaDTO entrada) throws Exception {
		RespuestaDTO respuesta = validateSearch(entrada);
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			Long cantidadRespuesta = estadoInstalacionDAO.getCantidad( (FiltroEstadoInstalacion) entrada.getObjetoEntrada(), entrada.getTamanioPagina(), entrada.getPaginaActual());
	...
	private RespuestaDTO validateChangeStatus(EntradaDTO entrada) {
		...
		EstadoInstalacionDTO estadoInstalacion = (EstadoInstalacionDTO) entrada.getObjetoEntrada();
		if(!entrada.getAccion().equals(AccionesCatalogosConstans.CREACION) && estadoInstalacion.getId()==null){
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_ID_NO_INFORMADO);
			return respuesta;
		}
