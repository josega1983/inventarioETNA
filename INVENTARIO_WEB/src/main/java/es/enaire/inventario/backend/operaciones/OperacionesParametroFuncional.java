...
public class OperacionesParametroFuncional {

	/**
	 * DAO de parametroFuncionales
	 */
	@Autowired
	private ParametroFuncionalDAO<ParametroFuncionalDTO> parametroFuncionalDAO;
	/**
	...
	private RespuestaDTO validateInfo(EntradaDTO entrada) {
		...
		if(!(entrada.getObjetoEntrada() instanceof ParametroFuncionalDTO)){
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_TIPO_ENTRADA_NO_ESPERADA);
			return respuesta;
		}

		if(entrada.getObjetoEntrada() instanceof ParametroFuncionalDTO) {
			ParametroFuncionalDTO parametroFuncional =(ParametroFuncionalDTO) entrada.getObjetoEntrada();
	...
	public RespuestaDTO info(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			...
			ParametroFuncionalDTO parametroFuncionalRespuesta = null;
			if(entrada.getObjetoEntrada() instanceof ParametroFuncionalDTO) {
				parametroFuncionalRespuesta = parametroFuncionalDAO.listById(((ParametroFuncionalDTO)entrada.getObjetoEntrada()).getId());
			}

			if(parametroFuncionalRespuesta != null) {
				Object[] objetoRespuesta={parametroFuncionalRespuesta};
	...
	public RespuestaDTO list(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			List<ParametroFuncionalDTO> parametroFuncionalRespuesta = parametroFuncionalDAO.list(entrada.getPaginaActual(), entrada.getTamanioPagina());
			Object[] objetoRespuesta = parametroFuncionalRespuesta.toArray();
			respuesta.setObjetoRespuesta(objetoRespuesta);
		}
		return respuesta;
	}
	...
	private RespuestaDTO validateEdit(EntradaDTO entrada) throws Exception {
		...
		ParametroFuncionalDTO parametroFuncional = (ParametroFuncionalDTO) entrada.getObjetoEntrada();
		if(parametroFuncional.getNombre() == null ||parametroFuncional.getNombre().isEmpty() ){
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_NOMBRE_NO_INFORMADO);
			return respuesta;
		}
		if(parametroFuncional.getConfiguracionParametroFuncionalList() == null || parametroFuncional.getConfiguracionParametroFuncionalList().size() ==0){
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_LISTA_CAMPOS_NO_INFORMADA);
			return respuesta;
			}
//   	else {
//			List<ConfiguracionParametroFuncionalDTO> configuracionParametroFuncionalList = parametroFuncional.getConfiguracionParametroFuncionalList();
//			for (ConfiguracionParametroFuncionalDTO configuracionParametroFuncionalDTO : configuracionParametroFuncionalList) {
//				if(configuracionParametroFuncionalDTO.getNombreCampo() == null || configuracionParametroFuncionalDTO.getNombreCampo().length() ==0){
//					respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_NOMBRE_CAMPO_NO_INFORMADO);
//					return respuesta;
//				}
//				if(configuracionParametroFuncionalDTO.getTipoCampoParametro() == null ||configuracionParametroFuncionalDTO.getTipoCampoParametro().getId() ==null){
//					respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_TIPO_CAMPO_NO_INFORMADO);
//					return respuesta;
//				}
//			}
//		}
		FiltroParametroFuncional filtro = new FiltroParametroFuncional();
		filtro.setNombre(parametroFuncional.getNombre());
		if(parametroFuncional.getId() != null){//es una actualizacion y hay que excluirlo
			filtro.setIdExcluir(parametroFuncional.getId());
		}
		List<ParametroFuncionalDTO> parametroFuncionalList = parametroFuncionalDAO.search(filtro, 0L, 10L);
	...
	public RespuestaDTO edit(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			ParametroFuncionalDTO parametroFuncionalEntrada = (ParametroFuncionalDTO) entrada.getObjetoEntrada();
			ParametroFuncionalDTO parametroFuncionalRespuesta = parametroFuncionalDAO.saveOrUpdate(parametroFuncionalEntrada);
			Object[] objetoRespuesta={parametroFuncionalRespuesta};
			respuesta.setObjetoRespuesta(objetoRespuesta);
		}
		return respuesta;
	...
	public RespuestaDTO reactivar(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			ParametroFuncionalDTO parametroFuncionalEntrada = (ParametroFuncionalDTO) entrada.getObjetoEntrada();
			ParametroFuncionalDTO parametroFuncionalRespuesta = parametroFuncionalDAO.reactivar(parametroFuncionalEntrada.getId());
			Object[] objetoRespuesta={parametroFuncionalRespuesta};
			respuesta.setObjetoRespuesta(objetoRespuesta);
		}
		return respuesta;
	...
	public RespuestaDTO delete(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			ParametroFuncionalDTO parametroFuncionalEntrada = (ParametroFuncionalDTO) entrada.getObjetoEntrada();
			parametroFuncionalDAO.delete(parametroFuncionalEntrada.getId());
		}
		return respuesta;
	}
	/**
	...
	private RespuestaDTO validateSearch(EntradaDTO entrada) {
		...
		if(!(entrada.getObjetoEntrada() instanceof FiltroParametroFuncional)) {
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_TIPO_ENTRADA_NO_ESPERADA);
			return respuesta;
		}
	...
	public RespuestaDTO search(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			List<ParametroFuncionalDTO> regionListRespuesta = parametroFuncionalDAO.search( (FiltroParametroFuncional) entrada.getObjetoEntrada(), entrada.getTamanioPagina(), entrada.getPaginaActual());
	...
	public RespuestaDTO getCantidad(EntradaDTO entrada) throws Exception {
		RespuestaDTO respuesta = validateSearch(entrada);
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			Long cantidadRespuesta = parametroFuncionalDAO.getCantidad( (FiltroParametroFuncional) entrada.getObjetoEntrada(), entrada.getTamanioPagina(), entrada.getPaginaActual());
	...
	private RespuestaDTO validateChangeStatus(EntradaDTO entrada) {
		...
		ParametroFuncionalDTO parametroFuncional = (ParametroFuncionalDTO) entrada.getObjetoEntrada();
		if(!entrada.getAccion().equals(AccionesCatalogosConstans.CREACION) && parametroFuncional.getId()==null){
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_ID_NO_INFORMADO);
			return respuesta;
		}
	...
	private RespuestaDTO changeStatusImpl(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			...
			if(entrada.getAccion().equals(AccionesCatalogosConstans.CREACION) ||entrada.getAccion().equals(AccionesCatalogosConstans.REACTIVACION)){
				parametroFuncionalEntrada.setActivo(AccionesCatalogosConstans.ACTIVO_SI);
				parametroFuncionalEntrada.setFechaBaja(null);
			}
			if(entrada.getAccion().equals(AccionesCatalogosConstans.ELIMINACION)){
				parametroFuncionalEntrada.setActivo(AccionesCatalogosConstans.ACTIVO_NO);
				parametroFuncionalEntrada.setFechaBaja(new Date());
			}
