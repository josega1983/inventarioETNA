...
public class OperacionesConfiguracionParametroFuncional {

	/**
	 * DAO de configuracion Parametro Funcionales
	 */
	@Autowired
	private ConfiguracionParametroFuncionalDAO<ConfiguracionParametroFuncionalDTO> configuracionParametroFuncionalDAO;
	/**
	...
	private RespuestaDTO validateInfo(EntradaDTO entrada) {
		...
		if(!(entrada.getObjetoEntrada() instanceof ConfiguracionParametroFuncionalDTO)){
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_TIPO_ENTRADA_NO_ESPERADA);
			return respuesta;
		}

		if(entrada.getObjetoEntrada() instanceof ConfiguracionParametroFuncionalDTO) {
			ConfiguracionParametroFuncionalDTO configuracionParametroFuncional =(ConfiguracionParametroFuncionalDTO) entrada.getObjetoEntrada();
	...
	public RespuestaDTO info(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			...
			ConfiguracionParametroFuncionalDTO configuracionParametroFuncionalRespuesta = null;
			if(entrada.getObjetoEntrada() instanceof ConfiguracionParametroFuncionalDTO) {
				configuracionParametroFuncionalRespuesta = configuracionParametroFuncionalDAO.listById(((ConfiguracionParametroFuncionalDTO)entrada.getObjetoEntrada()).getId());
			}

			if(configuracionParametroFuncionalRespuesta != null) {
				Object[] objetoRespuesta={configuracionParametroFuncionalRespuesta};
	...
	public RespuestaDTO list(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			List<ConfiguracionParametroFuncionalDTO> configuracionParametroFuncionalRespuesta = configuracionParametroFuncionalDAO.list(entrada.getPaginaActual(), entrada.getTamanioPagina());
			Object[] objetoRespuesta = configuracionParametroFuncionalRespuesta.toArray();
			respuesta.setObjetoRespuesta(objetoRespuesta);
		}
		return respuesta;
	}
	...
	private RespuestaDTO validateEdit(EntradaDTO entrada) throws Exception {
		...
		ConfiguracionParametroFuncionalDTO configuracionParametroFuncional = (ConfiguracionParametroFuncionalDTO) entrada.getObjetoEntrada();
		if(configuracionParametroFuncional.getNombreCampo() == null ||configuracionParametroFuncional.getNombreCampo().isEmpty() ){
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_NOMBRE_NO_INFORMADO);
			return respuesta;
		}
		FiltroConfiguracionParametroFuncional filtro = new FiltroConfiguracionParametroFuncional();
		filtro.setNombreCampo(configuracionParametroFuncional.getNombreCampo());
		if(configuracionParametroFuncional.getId() != null){//es una actualizacion y hay que excluirlo
			filtro.setIdExcluir(configuracionParametroFuncional.getId());
		}
		List<ConfiguracionParametroFuncionalDTO> configuracionParametroFuncionalList = configuracionParametroFuncionalDAO.search(filtro, 0L, 1L);
	...
	public RespuestaDTO edit(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			ConfiguracionParametroFuncionalDTO configuracionParametroFuncionalEntrada = (ConfiguracionParametroFuncionalDTO) entrada.getObjetoEntrada();
			ConfiguracionParametroFuncionalDTO configuracionParametroFuncionalRespuesta = configuracionParametroFuncionalDAO.saveOrUpdate(configuracionParametroFuncionalEntrada);
			Object[] objetoRespuesta={configuracionParametroFuncionalRespuesta};
			respuesta.setObjetoRespuesta(objetoRespuesta);
		}
		return respuesta;
	...
	public RespuestaDTO reactivar(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			ConfiguracionParametroFuncionalDTO configuracionParametroFuncionalEntrada = (ConfiguracionParametroFuncionalDTO) entrada.getObjetoEntrada();
			ConfiguracionParametroFuncionalDTO configuracionParametroFuncionalRespuesta = configuracionParametroFuncionalDAO.reactivar(configuracionParametroFuncionalEntrada.getId());
			Object[] objetoRespuesta={configuracionParametroFuncionalRespuesta};
			respuesta.setObjetoRespuesta(objetoRespuesta);
		}
		return respuesta;
	...
	public RespuestaDTO delete(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			ConfiguracionParametroFuncionalDTO configuracionParametroFuncionalEntrada = (ConfiguracionParametroFuncionalDTO) entrada.getObjetoEntrada();
			configuracionParametroFuncionalDAO.delete(configuracionParametroFuncionalEntrada.getId());
		}
		return respuesta;
	}
	/**
	...
	private RespuestaDTO validateSearch(EntradaDTO entrada) {
		...
		if(!(entrada.getObjetoEntrada() instanceof FiltroConfiguracionParametroFuncional)) {
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_TIPO_ENTRADA_NO_ESPERADA);
			return respuesta;
		}
	...
	public RespuestaDTO search(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			List<ConfiguracionParametroFuncionalDTO> regionListRespuesta = configuracionParametroFuncionalDAO.search( (FiltroConfiguracionParametroFuncional) entrada.getObjetoEntrada(), entrada.getTamanioPagina(), entrada.getPaginaActual());
	...
	private RespuestaDTO validateChangeStatus(EntradaDTO entrada) {
		...
		ConfiguracionParametroFuncionalDTO configuracionParametroFuncional = (ConfiguracionParametroFuncionalDTO) entrada.getObjetoEntrada();
		if(!entrada.getAccion().equals(AccionesCatalogosConstans.CREACION) && configuracionParametroFuncional.getId()==null){
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_ID_NO_INFORMADO);
			return respuesta;
		}
