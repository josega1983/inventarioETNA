...
public class OperacionesCentro {

	/**
	 * DAO de centroes
	 */
	@Autowired
	private CentroDAO<CentroDTO> centroDAO;
	/**
	...
	private RespuestaDTO validateInfo(EntradaDTO entrada) {
		...
		if(!(entrada.getObjetoEntrada() instanceof CentroDTO)){
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_TIPO_ENTRADA_NO_ESPERADA);
			return respuesta;
		}

		if(entrada.getObjetoEntrada() instanceof CentroDTO) {
			CentroDTO centro =(CentroDTO) entrada.getObjetoEntrada();
	...
	public RespuestaDTO info(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			...
			CentroDTO centroRespuesta = null;
			if(entrada.getObjetoEntrada() instanceof CentroDTO) {
				centroRespuesta = centroDAO.listById(((CentroDTO)entrada.getObjetoEntrada()).getId());
			}

			if(centroRespuesta != null) {
				Object[] objetoRespuesta={centroRespuesta};
	...
	public RespuestaDTO list(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			List<CentroDTO> centroRespuesta = centroDAO.list(entrada.getPaginaActual(), entrada.getTamanioPagina());
			Object[] objetoRespuesta = centroRespuesta.toArray();
			respuesta.setObjetoRespuesta(objetoRespuesta);
		}
		return respuesta;
	}
	...
	private RespuestaDTO validateEdit(EntradaDTO entrada) throws Exception {
		...
		FiltroCentro filtro = new FiltroCentro();
		filtro.setNombre(centro.getNombre());
		if(centro.getId() != null){//es una actualizacion y hay que excluirlo
			filtro.setIdExcluir(centro.getId());
		}
		List<CentroDTO> centroList = centroDAO.listByName(filtro.getNombre());
	...
	public RespuestaDTO edit(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			CentroDTO centroEntrada = (CentroDTO) entrada.getObjetoEntrada();
			CentroDTO centroRespuesta = centroDAO.saveOrUpdate(centroEntrada);
			Object[] objetoRespuesta={centroRespuesta};
			respuesta.setObjetoRespuesta(objetoRespuesta);
		}
		return respuesta;
	...
	public RespuestaDTO reactivar(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			CentroDTO centroEntrada = (CentroDTO) entrada.getObjetoEntrada();
			CentroDTO centroRespuesta = centroDAO.reactivar(centroEntrada.getId());
			Object[] objetoRespuesta={centroRespuesta};
			respuesta.setObjetoRespuesta(objetoRespuesta);
		}
		return respuesta;
	...
	public RespuestaDTO delete(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			CentroDTO centroEntrada = (CentroDTO) entrada.getObjetoEntrada();
			centroDAO.delete(centroEntrada.getId());
		}
		return respuesta;
	}
	/**
	...
	private RespuestaDTO validateSearch(EntradaDTO entrada) {
		...
		if(!(entrada.getObjetoEntrada() instanceof FiltroCentro)) {
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_TIPO_ENTRADA_NO_ESPERADA);
			return respuesta;
		}
	...
	public RespuestaDTO search(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			List<CentroDTO> regionListRespuesta = centroDAO.search( (FiltroCentro) entrada.getObjetoEntrada(), entrada.getTamanioPagina(), entrada.getPaginaActual());
	...
	public RespuestaDTO getCantidad(EntradaDTO entrada) throws Exception {
		RespuestaDTO respuesta = validateSearch(entrada);
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			Long cantidadRespuesta = centroDAO.getCantidad( (FiltroCentro) entrada.getObjetoEntrada(), entrada.getTamanioPagina(), entrada.getPaginaActual());
	...
	private RespuestaDTO validateChangeStatus(EntradaDTO entrada) {
		...
		CentroDTO centro = (CentroDTO) entrada.getObjetoEntrada();
		if(!entrada.getAccion().equals(AccionesCatalogosConstans.CREACION) && centro.getId()==null){
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_ID_NO_INFORMADO);
			return respuesta;
		}
	...
	private RespuestaDTO changeStatusImpl(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			...
			if(entrada.getAccion().equals(AccionesCatalogosConstans.CREACION) ||entrada.getAccion().equals(AccionesCatalogosConstans.REACTIVACION)){
				centroEntrada.setActivo(AccionesCatalogosConstans.ACTIVO_SI);
				centroEntrada.setFechaBaja(null);
			}
			if(entrada.getAccion().equals(AccionesCatalogosConstans.ELIMINACION)){
				centroEntrada.setActivo(AccionesCatalogosConstans.ACTIVO_NO);
				centroEntrada.setFechaBaja(new Date());
			}
