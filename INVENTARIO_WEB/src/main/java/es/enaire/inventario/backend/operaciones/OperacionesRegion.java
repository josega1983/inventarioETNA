...
public class OperacionesRegion {

	/**
	 * DAO de regiones
	 */
	@Autowired
	private RegionDAO<RegionDTO> regionDAO;
	/**
	...
	private RespuestaDTO validateInfo(EntradaDTO entrada) {
		...
		if(!(entrada.getObjetoEntrada() instanceof RegionDTO)){
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_TIPO_ENTRADA_NO_ESPERADA);
			return respuesta;
		}

		if(entrada.getObjetoEntrada() instanceof RegionDTO) {
			RegionDTO region =(RegionDTO) entrada.getObjetoEntrada();
	...
	public RespuestaDTO info(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			...
			RegionDTO regionRespuesta = null;
			if(entrada.getObjetoEntrada() instanceof RegionDTO) {
				regionRespuesta = regionDAO.listById(((RegionDTO)entrada.getObjetoEntrada()).getId());
			}

			if(regionRespuesta != null) {
				Object[] objetoRespuesta={regionRespuesta};
	...
	public RespuestaDTO list(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			List<RegionDTO> regionRespuesta = regionDAO.list(entrada.getPaginaActual(), entrada.getTamanioPagina());
			Object[] objetoRespuesta = regionRespuesta.toArray();
			respuesta.setObjetoRespuesta(objetoRespuesta);
		}
		return respuesta;
	}
	...
	private RespuestaDTO validateEdit(EntradaDTO entrada) throws Exception {
		...
		FiltroRegion filtro = new FiltroRegion();
		filtro.setNombre(region.getNombre());
		if(region.getId() != null){//es una actualizacion y hay que excluirla
			filtro.setIdExcluir(region.getId());
		}
		List<RegionDTO> regionList = regionDAO.listByName(filtro.getNombre());
	...
	public RespuestaDTO edit(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			RegionDTO regionEntrada = (RegionDTO) entrada.getObjetoEntrada();
			RegionDTO regionRespuesta = regionDAO.saveOrUpdate(regionEntrada);
			Object[] objetoRespuesta={regionRespuesta};
			respuesta.setObjetoRespuesta(objetoRespuesta);
		}
		return respuesta;
	...
	public RespuestaDTO reactivar(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			RegionDTO regionEntrada = (RegionDTO) entrada.getObjetoEntrada();
			RegionDTO regionRespuesta = regionDAO.reactivar(regionEntrada.getId());
			Object[] objetoRespuesta={regionRespuesta};
			respuesta.setObjetoRespuesta(objetoRespuesta);
		}
		return respuesta;
	...
	public RespuestaDTO delete(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			RegionDTO regionEntrada = (RegionDTO) entrada.getObjetoEntrada();
			regionDAO.delete(regionEntrada.getId());
		}
		return respuesta;
	}
	...
	private RespuestaDTO validateSearch(EntradaDTO entrada) {
		...
		if(!(entrada.getObjetoEntrada() instanceof FiltroRegion)) {
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_TIPO_ENTRADA_NO_ESPERADA);
			return respuesta;
		}
	...
	public RespuestaDTO search(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			List<RegionDTO> regionListRespuesta = regionDAO.search( (FiltroRegion) entrada.getObjetoEntrada(), entrada.getTamanioPagina(), entrada.getPaginaActual());
	...
	public RespuestaDTO getCantidad(EntradaDTO entrada) throws Exception {
		RespuestaDTO respuesta = validateSearch(entrada);
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			Long cantidadRespuesta = regionDAO.getCantidad( (FiltroRegion) entrada.getObjetoEntrada(), entrada.getTamanioPagina(), entrada.getPaginaActual());
	...
	private RespuestaDTO validateChangeStatus(EntradaDTO entrada) {
		...
		RegionDTO region = (RegionDTO) entrada.getObjetoEntrada();
		if(!entrada.getAccion().equals(AccionesCatalogosConstans.CREACION) && region.getId()==null){
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_ID_NO_INFORMADO);
			return respuesta;
		}
	...
	private RespuestaDTO changeStatusImpl(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			...
			if(entrada.getAccion().equals(AccionesCatalogosConstans.CREACION) ||entrada.getAccion().equals(AccionesCatalogosConstans.REACTIVACION)){
				regionEntrada.setActivo(AccionesCatalogosConstans.ACTIVO_SI);
				regionEntrada.setFechaBaja(null);
			}
			if(entrada.getAccion().equals(AccionesCatalogosConstans.ELIMINACION)){
				regionEntrada.setActivo(AccionesCatalogosConstans.ACTIVO_NO);
				regionEntrada.setFechaBaja(new Date());
			}
