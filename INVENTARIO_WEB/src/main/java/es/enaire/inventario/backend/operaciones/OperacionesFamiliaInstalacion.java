...
public class OperacionesFamiliaInstalacion {

	/**
	 * DAO de Familia de instalaciones
	 */
	@Autowired
	private FamiliaInstalacionDAO<FamiliaInstalacionDTO> familiaInstalacionDAO;
	/**
	...
	private RespuestaDTO validateInfo(EntradaDTO entrada) {
		...
		if(!(entrada.getObjetoEntrada() instanceof FamiliaInstalacionDTO)){
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_TIPO_ENTRADA_NO_ESPERADA);
			return respuesta;
		}

		if(entrada.getObjetoEntrada() instanceof FamiliaInstalacionDTO) {
			FamiliaInstalacionDTO familiaInstalacion =(FamiliaInstalacionDTO) entrada.getObjetoEntrada();
	...
	public RespuestaDTO info(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			...
			FamiliaInstalacionDTO familiaInstalacionRespuesta = null;
			if(entrada.getObjetoEntrada() instanceof FamiliaInstalacionDTO) {
				familiaInstalacionRespuesta = familiaInstalacionDAO.listById(((FamiliaInstalacionDTO)entrada.getObjetoEntrada()).getId());
			}

			if(familiaInstalacionRespuesta != null) {
				Object[] objetoRespuesta={familiaInstalacionRespuesta};
	...
	public RespuestaDTO list(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			List<FamiliaInstalacionDTO> familiaInstalacionRespuesta = familiaInstalacionDAO.list(entrada.getPaginaActual(), entrada.getTamanioPagina());
			Object[] objetoRespuesta = familiaInstalacionRespuesta.toArray();
			respuesta.setObjetoRespuesta(objetoRespuesta);
		}
		return respuesta;
	}
	...
	private RespuestaDTO validateEdit(EntradaDTO entrada) throws Exception {
		...
		FamiliaInstalacionDTO familiaInstalacion = (FamiliaInstalacionDTO) entrada.getObjetoEntrada();
		if(familiaInstalacion.getNombre() == null ||familiaInstalacion.getNombre().isEmpty() ){
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_NOMBRE_NO_INFORMADO);
			return respuesta;
		}
		FiltroFamiliaInstalacion filtro = new FiltroFamiliaInstalacion();
		filtro.setNombre(familiaInstalacion.getNombre());
		if(familiaInstalacion.getId() != null){//es una actualizacion y hay que excluirlo
			filtro.setIdExcluir(familiaInstalacion.getId());
		}
		List<FamiliaInstalacionDTO> familiaInstalacionList = familiaInstalacionDAO.listByName(filtro.getNombre());
	...
	public RespuestaDTO edit(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			FamiliaInstalacionDTO familiaInstalacionEntrada = (FamiliaInstalacionDTO) entrada.getObjetoEntrada();
			FamiliaInstalacionDTO familiaInstalacionRespuesta = familiaInstalacionDAO.saveOrUpdate(familiaInstalacionEntrada);
			Object[] objetoRespuesta={familiaInstalacionRespuesta};
			respuesta.setObjetoRespuesta(objetoRespuesta);
		}
		return respuesta;
	...
	public RespuestaDTO reactivar(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			FamiliaInstalacionDTO familiaInstalacionEntrada = (FamiliaInstalacionDTO) entrada.getObjetoEntrada();
			FamiliaInstalacionDTO familiaInstalacionRespuesta = familiaInstalacionDAO.reactivar(familiaInstalacionEntrada.getId());
			Object[] objetoRespuesta={familiaInstalacionRespuesta};
			respuesta.setObjetoRespuesta(objetoRespuesta);
		}
		return respuesta;
	...
	public RespuestaDTO delete(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			FamiliaInstalacionDTO familiaInstalacionEntrada = (FamiliaInstalacionDTO) entrada.getObjetoEntrada();
			familiaInstalacionDAO.delete(familiaInstalacionEntrada.getId());
		}
		return respuesta;
	}
	/**
	...
	private RespuestaDTO validateSearch(EntradaDTO entrada) {
		...
		if(!(entrada.getObjetoEntrada() instanceof FiltroFamiliaInstalacion)) {
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_TIPO_ENTRADA_NO_ESPERADA);
			return respuesta;
		}
	...
	public RespuestaDTO search(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			List<FamiliaInstalacionDTO> regionListRespuesta = familiaInstalacionDAO.search( (FiltroFamiliaInstalacion) entrada.getObjetoEntrada(), entrada.getTamanioPagina(), entrada.getPaginaActual());
	...
	public RespuestaDTO getCantidad(EntradaDTO entrada) throws Exception {
		RespuestaDTO respuesta = validateSearch(entrada);
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			Long cantidadRespuesta = familiaInstalacionDAO.getCantidad( (FiltroFamiliaInstalacion) entrada.getObjetoEntrada(), entrada.getTamanioPagina(), entrada.getPaginaActual());
	...
	private RespuestaDTO validateChangeStatus(EntradaDTO entrada) {
		...
		FamiliaInstalacionDTO familiaInstalacion = (FamiliaInstalacionDTO) entrada.getObjetoEntrada();
		if(!entrada.getAccion().equals(AccionesCatalogosConstans.CREACION) && familiaInstalacion.getId()==null){
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_ID_NO_INFORMADO);
			return respuesta;
		}
	...
	private RespuestaDTO changeStatusImpl(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			...
			if(entrada.getAccion().equals(AccionesCatalogosConstans.CREACION) ||entrada.getAccion().equals(AccionesCatalogosConstans.REACTIVACION)){
				familiaInstalacionEntrada.setActivo(AccionesCatalogosConstans.ACTIVO_SI);
				familiaInstalacionEntrada.setFechaBaja(null);
			}
			if(entrada.getAccion().equals(AccionesCatalogosConstans.ELIMINACION)){
				familiaInstalacionEntrada.setActivo(AccionesCatalogosConstans.ACTIVO_NO);
				familiaInstalacionEntrada.setFechaBaja(new Date());
			}
