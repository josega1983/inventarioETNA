...
public class OperacionesLocalizacion {
	/**
	 * DAO de localizacion
	 */
	@Autowired
	private LocalizacionDAO<LocalizacionDTO> localizacionDAO;
	/**
	...
	private RespuestaDTO validateInfo(EntradaDTO entrada) {
		...
		if(!(entrada.getObjetoEntrada() instanceof LocalizacionDTO)){
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_TIPO_ENTRADA_NO_ESPERADA);
			return respuesta;
		}

		if(entrada.getObjetoEntrada() instanceof LocalizacionDTO) {
			LocalizacionDTO localizacion =(LocalizacionDTO) entrada.getObjetoEntrada();
	...
	public RespuestaDTO info(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			...
			LocalizacionDTO localizacionRespuesta = null;
			if(entrada.getObjetoEntrada() instanceof LocalizacionDTO) {
				localizacionRespuesta = localizacionDAO.listById(((LocalizacionDTO)entrada.getObjetoEntrada()).getId());
			}

			if(localizacionRespuesta != null) {
				Object[] objetoRespuesta={localizacionRespuesta};
	...
	public RespuestaDTO list(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			List<LocalizacionDTO> localizacionRespuesta = localizacionDAO.list(entrada.getPaginaActual(), entrada.getTamanioPagina());
			Object[] objetoRespuesta = localizacionRespuesta.toArray();
			respuesta.setObjetoRespuesta(objetoRespuesta);
		}
		return respuesta;
	}
	...
	private RespuestaDTO validateEdit(EntradaDTO entrada) throws Exception {
		...
		FiltroLocalizacion filtro = new FiltroLocalizacion();
		filtro.setNombre(localizacion.getNombre());
		if(localizacion.getId() != null){//es una actualizacion y hay que excluirlo
			filtro.setIdExcluir(localizacion.getId());
		}
		List<LocalizacionDTO> localizacionList = localizacionDAO.listByName(filtro.getNombre());
	...
	public RespuestaDTO edit(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			LocalizacionDTO localizacionEntrada = (LocalizacionDTO) entrada.getObjetoEntrada();
			LocalizacionDTO localizacionRespuesta = localizacionDAO.saveOrUpdate(localizacionEntrada);
			Object[] objetoRespuesta={localizacionRespuesta};
			respuesta.setObjetoRespuesta(objetoRespuesta);
		}
		return respuesta;
	...
	public RespuestaDTO reactivar(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			LocalizacionDTO localizacionEntrada = (LocalizacionDTO) entrada.getObjetoEntrada();
			LocalizacionDTO localizacionRespuesta = localizacionDAO.reactivar(localizacionEntrada.getId());
			Object[] objetoRespuesta={localizacionRespuesta};
			respuesta.setObjetoRespuesta(objetoRespuesta);
		}
		return null;
	...
	public RespuestaDTO delete(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			LocalizacionDTO localizacionEntrada = (LocalizacionDTO) entrada.getObjetoEntrada();
			localizacionDAO.delete(localizacionEntrada.getId());
		}
		return respuesta;
	}
	/**
	...
	private RespuestaDTO validateSearch(EntradaDTO entrada) {
		...
		if(!(entrada.getObjetoEntrada() instanceof FiltroLocalizacion)) {
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_TIPO_ENTRADA_NO_ESPERADA);
			return respuesta;
		}
	...
	public RespuestaDTO search(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			List<LocalizacionDTO> regionListRespuesta = localizacionDAO.search( (FiltroLocalizacion) entrada.getObjetoEntrada(), entrada.getTamanioPagina(), entrada.getPaginaActual());
	...
	public RespuestaDTO getCantidad(EntradaDTO entrada) throws Exception {
		RespuestaDTO respuesta = validateSearch(entrada);
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			Long cantidadRespuesta = localizacionDAO.getCantidad( (FiltroLocalizacion) entrada.getObjetoEntrada(), entrada.getTamanioPagina(), entrada.getPaginaActual());
	...
	private RespuestaDTO validateChangeStatus(EntradaDTO entrada) {
		...
		LocalizacionDTO localizacion = (LocalizacionDTO) entrada.getObjetoEntrada();
		if(!entrada.getAccion().equals(AccionesCatalogosConstans.CREACION) && localizacion.getId()==null){
			respuesta.setCodigoRespuesta(CodigosRespuestaConstants.ERROR_ID_NO_INFORMADO);
			return respuesta;
		}
	...
	private RespuestaDTO changeStatusImpl(EntradaDTO entrada) throws Exception {
		...
		if (respuesta.getCodigoRespuesta().equals(CodigosRespuestaConstants.RESPUESTA_OK)) {
			...
			if(entrada.getAccion().equals(AccionesCatalogosConstans.CREACION) ||entrada.getAccion().equals(AccionesCatalogosConstans.REACTIVACION)){
				localizacionEntrada.setActivo(AccionesCatalogosConstans.ACTIVO_SI);
				localizacionEntrada.setFechaBaja(null);
			}
			if(entrada.getAccion().equals(AccionesCatalogosConstans.ELIMINACION)){
				localizacionEntrada.setActivo(AccionesCatalogosConstans.ACTIVO_NO);
				localizacionEntrada.setFechaBaja(new Date());
			}
